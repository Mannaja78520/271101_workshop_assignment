...
while not (check_user_input(comport)):
    print('Please enter a number not string')
    comport = input('Enter the arduino board COM port: ')
    # comport = input('Enter the arduino board USB port: ')

# board=pyfirmata.Arduino('COM'+comport) #window
board=pyfirmata.Arduino('/dev/ttyUSB'+comport) #Linux
...
def led(ledCommand,led_1,led_2,led_3,led_4,led_5):#creat condition to controll digital out put
    
    led_1.write(ledCommand[0])
    led_2.write(ledCommand[1])
    led_3.write(ledCommand[2])
    led_4.write(ledCommand[3])
    led_5.write(ledCommand[4])
